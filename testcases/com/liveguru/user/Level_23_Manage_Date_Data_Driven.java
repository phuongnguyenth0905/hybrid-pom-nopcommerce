...
import com.liveguru.data.UserDataModel;
...
public class Level_23_Manage_Date_Data_Driven extends AbstractTest {

	WebDriver driver;
	int randomNum = getRandomNumber();
	UserDataModel userData = UserDataModel.getValue(GlobalConstans.ROOT_FOLDER + "\\testdata\\com\\liveguru\\data\\UserData.json");

	String discountCode, productNameSonyXperia, today, productPriceSonyXperia, subTotalPrice, discountPrice, flatRate, grandTotal;
	String country, state, zipCode, adress, city, phone, cardName, cardMonth, cardYear, cardCVV;
...
//		cardName = firtName + "" + lastName;
	...
	public void TC_01_Register_To_System() {
		...
		log.info("Register - Step 04 : Input To First Name textbox with value: " + userData.getFirstname());
		registerPage.inputToFirstNameTextbox(userData.getFirstname());

		log.info("Register - Step 05 : Input To Last Name textbox with value: " + userData.getLastname());
		registerPage.inputToLastNameTextbox(userData.getLastname());

		log.info("Register - Step 06 : Input To Email textbox with value: " + userData.getEmail() + randomNum + "@live.com");
		registerPage.inputToEmailTexbox(userData.getEmail() + randomNum + "@live.com");

		log.info("Register - Step 07 : Input To Password textbox with value: " + userData.getPassword());
		registerPage.inputToPasswordTexbox(userData.getPassword());
		registerPage.inputToConfirmPasswordTexbox(userData.getPassword());
	...
	public void TC_02_Login_To_System() {
		...
		log.info("Login - Step 02 : Input To Email textbox with value: " + userData.getEmail() + randomNum + "@live.com");
		loginPage.inputToEmailTextbox(userData.getEmail() + randomNum + "@live.com");
	...
	public void TC_03_Account_Information() {
		...
		verifyEquals(myAccountInformationPage.getFirstnameTextboxValue(), userData.getFirstname());

		log.info("Account_Infor - Step 03: Vefify Lastname value is correct");
		verifyEquals(myAccountInformationPage.getLastnameTextboxValue(), userData.getLastname());

		log.info("Account_Infor - Step 04: Vefify Email value is correct");
		verifyEquals(myAccountInformationPage.getEmailTextboxValue(), userData.getEmail() + randomNum + "@live.com");
	}
...
//		checkoutPage.removeItemProduct();
	...
	public void TC_10_Order_Product() {
		...
		checkoutPage.selectToCartTypeDropdown(userData.getCardType());

		log.info("TC_10_Order_Product - Step 25: Input to Card Number textbox");
		checkoutPage.inputToCardNumberTextbox(userData.getCardNumber());
		...
		verifyTrue(myDashboardPage.isOrderInformationDisplayed(orderID, today, userData.getFirstname() + "" + userData.getLastname(), grandTotal, "Pending"));
	}
